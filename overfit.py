...
from activation_for_mnist import FiveLayorNet
import numpy as np
...
network = FiveLayorNet(input_size=784,hidden_size=50,output_size=10)
...
for epoch in range(epochs):
    for iter in range(iters_per_epoch):
        ...
        #勾配
        loss = network.loss_with_weight_decay(x_batch,t_batch)
        grads = network.gradient(x_batch,t_batch,weight_decay=True)
...
plt.legend()
plt.xlabel('epoch')
plt.ylabel('accuracy_with_weight_decay')
plt.show()    
